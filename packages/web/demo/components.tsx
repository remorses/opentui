// Shared components for web demos
...
export const exampleDiff = `--- a/server.ts
+++ b/server.ts
@@ -1,8 +1,12 @@
...
export type Tab = "overview" | "diff" | "scroll" | "colors"

export function Header({ tab, spinner, title }: { tab: Tab; spinner: string; title: string }) {
  return (
    <box flexDirection="row" marginBottom={1}>
      <text fg={theme.accent}>
        <strong>{title} {spinner}</strong>
      </text>
...
export function App({ title, overviewTab }: { title: string; overviewTab: ReactNode }) {
  ...
  return (
    <box flexDirection="column" padding={1} backgroundColor={theme.bg}>
      <Header tab={tab} spinner={SPINNERS[spinnerIdx]} title={title} />
